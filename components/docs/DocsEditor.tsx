...
import { Save, FileText, Clock, Type, Heading1, Heading2, Heading3, Quote, List, ListOrdered, Table, Image, Video, Info, Check, FileText as FileTextIcon, Lightbulb, AlertTriangle, AlertCircle, ChevronDown, ChevronRight, CreditCard, Code, Table as TabsIcon, Star } from 'lucide-react';
...
const CalloutElement = ({ attributes, children, element }: any) => {
  const variants = {
    ...
    [ELEMENT_NOTE_CALLOUT]: { icon: FileTextIcon, bg: 'bg-gray-50', border: 'border-gray-200', text: 'text-gray-800' },
...
const CardElement = ({ attributes, children, element }: any) => (
  <Card {...attributes} className="my-4">
    <CardContent className="p-4">
      <h4 className="font-medium mb-2">{element.title || 'Card Title'}</h4>
      {children}
    </CardContent>
  </Card>
);
...
const SlashCommandMenu = ({ isOpen, onClose, onSelect, position, editor }: any) => {
  if (!isOpen) return null;

  return (
    <div
      className="fixed z-50 w-80 bg-white border rounded-lg shadow-lg max-h-96 overflow-y-auto"
...
export function DocsEditor({ className }: DocsEditorProps) {
  ...
  // Convert markdown content to Plate.js format
  const convertMarkdownToPlate = useCallback((markdown: string) => {
    if (!markdown || markdown.trim() === '') {
      return [{ type: ELEMENT_PARAGRAPH, children: [{ text: '' }] }];
    }

    // Simple markdown to Plate conversion
    ...
    return plateNodes.length > 0 ? plateNodes : [{ type: ELEMENT_PARAGRAPH, children: [{ text: '' }] }];
  }, []);

  // Convert Plate.js content to markdown
  const convertPlateToMarkdown = useCallback((plateValue: any[]) => {
    try {
      return serializeMdx(plateValue);
    } catch (error) {
      console.error('Error converting to MDX:', error);
      // Fallback to simple conversion
      return plateValue.map((node: any) => {
        if (node.type === ELEMENT_H1) {
          return `# ${node.children[0]?.text || ''}`;
        } else if (node.type === ELEMENT_H2) {
          return `## ${node.children[0]?.text || ''}`;
        } else if (node.type === ELEMENT_H3) {
          return `### ${node.children[0]?.text || ''}`;
        } else if (node.type === ELEMENT_BLOCKQUOTE) {
          return `> ${node.children[0]?.text || ''}`;
        } else {
          return node.children[0]?.text || '';
        }
  ...
  const handleEditorChange = useCallback((value: any[]) => {
    ...
    if (state.currentPage) {
      const markdownContent = convertPlateToMarkdown(value);
      actions.updatePageContent(state.currentPage.id, markdownContent);
    }
  }, [state.currentPage, actions, convertPlateToMarkdown]);
  ...
  useEffect(() => {
    if (state.currentPage) {
      const plateContent = convertMarkdownToPlate(state.currentPage.content);
      setEditorValue(plateContent);
    }
  }, [state.currentPage?.id, convertMarkdownToPlate]);
